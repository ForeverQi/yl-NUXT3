            ...
            <div data-v-3ebd8327 class="el-dialog__wrapper" style="z-index: 2009">
                <div role="dialog" aria-modal="true" aria-label="dialog" class="el-dialog" style="margin-top: 15vh; z-index: 2001">
                    ...
                    <div class="el-dialog__body">
                        <img data-v-3ebd8327 :src="choseSrc" />
                    </div>
                </div>
            </div>
    ...
    methods: {
        ...
        inputChange() {
            ...
            if (/Android|webOS|iPhone|iPod|iPad|BlackBerry/i.test(navigator.userAgent)) {
                picRotatUtil.checkDirection(files[0], (data) => {
        ...
        beforeUpload(files) {
            ...
            if (files && Object.keys(files).length > 0) {
                for (let file of Object.values(files)) {
                    if (this.suffixs) {
                        let a = this.suffixs.filter((item) => {
                        ...
                        if (
                            ...
                            // this.showDialog(`只能上传${this.suffixs}文件`);
                            return false;
                        }
                    }
                    if (this.size) {
                        ...
                        if (file.size > this.size) {
                            ...
                            // this.showDialog(`文件大小不可超过${this.size / 1024 / 1024}MB`);
                            return false;
                        }
                    }
                    // if (this.limit) {
                    //     if (Object.values(files).length > this.limit) {
                    //         console.log(this.fileList, 154);
                    //         this.$baseUI.alert(`最多可添加${this.limit}张图片`);
                    //         // this.showDialog(`文件大小不可超过${this.size / 1024 / 1024}MB`);
                    //         return false;
                    //     }
                    // }
