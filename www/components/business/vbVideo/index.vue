    ...
    methods: {
        ...
        playVid(vidObj = {}, option = {}, isErrCatch = true) {
            return Promise.resolve()
                ...
                .then(() => {
                    const userInfo = this.$storage.get('loginStatus') || { userYoulunum: '' };
                    ...
                    if (vidObj.noToken === true) {
                        return Promise.resolve(option_);
                    }
                    return this.getOption(option_, isErrCatch);
                })
                .then((res) => {
        ...
        getOption(option, isErrCatch = true) {
            /**
             ...
             * }
             */
            const { coursewareId, goodsId } = option;
        ...
        mcpVideoWebToken(opt, isErrCatch = true) {
            ...
            let data = {
                url: 'video/mcp_video_web_token',
                params: {
                    ...opt,
                },
                isCustom: true,
            };
            const pros = this.httpClient(data).then((res) => {
                return res;
            });
            if (isErrCatch) {
