        ...
        <div class="understand-content-left">
            ...
            <ul class="chapter-ul">
                <li v-for="item in dataChapterTree" :key="item.chapterId">
                    ...
                    <ul class="knob-ul" v-show="item.childChapterList && item.childChapterList.length > 0 && item.isFold">
                        <li
                        ...
                        >
                            <div class="one-node">
                                <span></span>
                                <div class="node-title">{{ item1.chapterTitle }}</div>
                            </div>
                        </li>
                    </ul>
                </li>
        ...
        <div class="understand-content-right">
            ...
            <ul class="test-centre-ul" v-if="page.pageCount > 0">
                ...
                <li v-for="item in dataKnowledgeList" :key="item.knowledgeId">
                    ...
                    <div class="second">
                        <img src="./asset/images/star.png" v-for="index of item.knowledgeStarLevel" :key="index" alt="" />
                    </div>
                    <div class="third">
            ...
            <div class="empty-tip" v-else>
                ...
                <div v-if="isSearch">没有找到要搜索的考点，请输入正确的考点内容进行搜索哦！</div>
                <div v-else>此处暂时没有知识点内容，去看看其他章节的知识点吧~</div>
            </div>
            <pagination
...
import '@/api/ks/api..chapter-level-tree.js';
import '@/api/ks/api..knowledge-understand-list.js';
    ...
    methods: {
        ...
        handleNode(val) {
            ...
            this.$yiguan.learningCenter('web_learning_center_check_knowledge', {
                college_name: '',
                page_name: '考点秒懂页面',
                project_name: this.currentProject.projectName,
        ...
        changeKnowledge() {},
        getClassSubjects() {
            //cc/user/study-tool/subject/list
            this.$callApi('cc/study-tool-subject-list', {
                projectId: this.currentProject.projectId,
                studyTool: 'examSite',
                onsuccess: (result) => {
                    this.dataSubject = result;
                    this.currentSubject = result[0] || null;
                },
                onerror: this.$niceloo.api.handleFailure,
            });
        },
        getChapterTree() {
            //ks/chapter/level/tree/list
            this.$callApi('ks/chapter-level-tree', {
                ...
                onsuccess: (result) => {
                    ...
                    this.currentChapter = result[0].childChapterList[0];
                },
                onerror: this.$niceloo.api.handleFailure,
            });
        },
        getKnowledgeUnderstand() {
            //ks/knowledge/understand/list
            this.$callApi('ks/knowledge-understand-list', {
                subjectId: this.currentSubject.subjectId,
