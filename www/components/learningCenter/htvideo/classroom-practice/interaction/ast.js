export default function html2AST(html) {
    ...
    const selfCloseTags = ['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'];
    const bufArray = [];
    const results = {
        node: 'root',
        child: [],
    ...
    function parseStartTag(tag, tagName, rest) {
        ...
        const ds = {};
        const attrs = [];
        let unary = !!arguments[7] || selfCloseTags.includes(tagName);
        console.log(tag, tagName, unary);
        const node = {
            node: 'element',
            tag: tagName,
        };
