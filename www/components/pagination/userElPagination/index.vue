        ...
        <div class="pagination">
            ...
            <div class="plugins items-box">
                <div
                    ...
                    @click="currentChange(form.currentPage - 1)"
                >
                    {{ prevText }}
                </div>
                <template v-if="layout.indexOf('pager') !== -1">
                    <div
                        ...
                        @click="currentChange(item.pageIndex)"
                    >
                        {{ item.title }}
                    </div>
                </template>
            ...
            <div v-if="layout.indexOf('jumper') !== -1" class="plugins">
                <span>跳至</span>
                <input type="number" class="input-jumper" v-model="form.inputPage" min="1" @blur="keyupEnter" @keyup.enter="keyupEnter" />
                <span>页</span>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
         ...
         * @returns {number} obj.title 当前项的展示名称
         */
        pageTotal() {
    ...
    methods: {
        ...
        // 输入框跳转
        keyupEnter() {
            let numPage = Number(this.form.inputPage);
            // 输入的内容无法解析成数字
            if (Number.isNaN(numPage)) {
                numPage = this.pageTotal;
                this.form.inputPage = this.pageTotal;
                // return;
            }
            numPage = Math.floor(numPage);
            // 输入的数字超过可选范围
            if (numPage < 1 || numPage > this.pageTotal) {
                this.form.inputPage = this.pageTotal;
                // return;
            }
            // 输入内容正常,执行跳转
            this.currentChange(Math.floor(this.form.inputPage));
        },
    },
};
</script>
