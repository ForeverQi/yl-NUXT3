<template>
    <div class="edu-mask" v-show="show">
        <div class="info-wrap">
            ...
            <p class="tips-card">请输入此学员身份证号后六位</p>
            <form class="user-form">
                <input
                    v-model.trim="item.num"
                    class="item-val"
                    v-for="(item, index) in cardSuffixNum"
                    :key="index"
                    :ref="'input' + index"
...
<script>
import '@/api/edu/api..getstuidcard';
...
export default {
    ...
    computed: {
        disabled() {
            return this.cardSuffixNum.some((item) => item.num.length <= 0);
        },
    },
    watch: {
        cardSuffixNum: {
        ...
        show(newV) {
            if (newV) {
                this.cardSuffixNum.forEach((item) => (item.num = ''));
            }
        },
    },
    methods: {
        ...
        getIdCard() {
            return this.$callApi('edu/getstuidcard', {
                idCard: this.cardSuffixNum.map((item) => item.num).join(''),
