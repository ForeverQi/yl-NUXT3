<template>
    <ul class="payContent">
        <li class="payLi" v-for="itemData in orderList" :key="itemData.a">
            ...
            <img v-else-if="itemData.orderSubtype == 'REP' && itemData.classOrderSubType !== 'YAN'" src="./asset/images/stress.png" alt="" class="botIcon" />
            <img v-else-if="itemData.orderSubtype == 'REP' && itemData.classOrderSubType === 'YAN'" src="./asset/images/yan.png" alt="" class="botIcon" />
            ...
            <div class="liBot">
                ...
                <div class="totalMoney totalMoney-inline preferential-price" v-if="itemData.orderDiscountamount > 0">
                    优惠金额:
                ...
                <div class="buttonWrap" v-if="itemData.orderCreateddate && milepost(itemData.orderCreateddate)">
                    ...
                    <input type="button" class="detailButton confirm" v-else @click="toDetais(itemData)" value="确认个人信息" />
                    <input
                        type="button"
                        class="detailButton topay collage-order"
                        v-if="isShowConfirmStage(itemData)"
                        @click="handleSchoolOrderConfirm(itemData)"
                        value="确认考研院校"
                    />
                    <input
                        type="button"
                        class="detailButton topay collage-order"
                        v-else-if="isShowConfirmStageDetail(itemData)"
                        @click="handleSchoolOrderDetail(itemData)"
                        value="查看考研院校确认单"
                    />
                    ...
                    <input
                        ...
                        value="立即支付"
                    />
                    <span class="detailButton" v-if="itemData.showSelfLearn" @click="doSelfLearn(itemData)">申请续学</span>
                </div>
            </div>
        </li>
    </ul>
...
export default {
    ...
    methods: {
        ...
        doSelfLearn(item) {
            // 自助续学
            this.$emit('doSelfLearn', item.selfLearnClassList, item);
        },
        handleSchoolOrderConfirm(item) {
            this.$emit('toSchoolOrderConfirm', item);
        },
        handleSchoolOrderDetail(item) {
            this.$emit('toSchoolOrderDetail', item);
        },
        isShowConfirmStage(item) {
            if (item.orderValidstatus === 'N') {
                return false;
            } else if (item.orderSubtype == '4') {
                return false;
            }
            return item.confirmStage === 'CollegeConfirming';
        },
        isShowConfirmStageDetail(item) {
            if (item.orderValidstatus === 'N') {
                return false;
            } else if (item.orderSubtype == '4') {
                return false;
            }
            return item.confirmStage === 'CollegeConfirmed';
        },
    },
};
</script>
...
.payLi {
    ...
    .liBot {
        ...
        .confirm {
            ...
            background-image: url('./asset/images/edit.png');
            background-repeat: no-repeat;
