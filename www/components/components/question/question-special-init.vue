<template>
    <div ref="questionInit" class="question-init">
        ...
        <div v-for="(item, index) in showQuestionList" :key="index" class="question-item" :class="{ separate: type }">
            <div class="title-box">
...
export default {
    ...
    computed: {
        ...
        /**
         * @returns {Array} 显示试题列表
         */
        showQuestionList() {
            if (this.treeList.length === 0) {
                return [];
            }
            console.log(this.treeList[this.current], '@@---');
            return [this.treeList[this.current]];
        },
        // 将传入的questionList转换成组件可用的tree结构
        /**
         * @returns {object} 显示试题列表
         */
        treeList() {
    ...
    methods: {
        ...
        initQuestionList() {
            var titleIndex = 0;
            this.questionList = this.list.map((_item, index) => {
                const item = Object.assign({}, _item);
                titleIndex += index === 0 ? 1 : _item.groupType === this.list[index - 1].groupType ? 0 : 1;
