...
export default {
    ...
    methods: {
            ...
            return this.requestWrapper('order/pc_trade_pay', {
                ...
                if (bizType === 'F') {
                    ...
                    this.$yiguan.track('youlu_wk_pay', {
                        ...
                        course_type: '体验课',
                        order_type: '流量类订单',
                        payable_price: this.experienceClass.coursePrice / 100 + '',
                        pay_type: '支付宝',
                        page_type: '资源详情页',
        ...
        orderingOrderDetail() {
            return this.requestWrapper('order/ordering_studentorder_microclass_orderdetail_post', {
                orderNo: this.orderNo,
            })
                .then((res) => {
        ...
        trackPaySuccess() {
            this.trackWebYouluPaySuccess({
                course_name: this.course?.courseName ?? '',
                project_position: this.course?.projectId ?? '',
                subject_name: this.course?.subjectId ?? '',
                course_type: '体验课',
                pay_type: (this.paymodeCode === 'WX' && '微信支付') || (this.paymodeCode === 'ALIPAY' && '支付宝支付') || '未知类型',
                payable_price: this.order.orderTotalamount / 100,
                page_type: this.pageSource || '体验课详情页',
                page_name: document.title,
            });
        },
        trackPayLaunch() {
            this.trackWebYouluWkPay({
                course_name: this.course?.courseName ?? '',
                project_position: this.course?.projectId ?? '',
                subject_name: this.course?.subjectId ?? '',
                course_type: '体验课',
