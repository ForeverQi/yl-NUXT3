<template>
    <div style="width: 100%; height: 100%; position: relative" :id="'warp' + componentId">
...
<script>
import Platform from '~/plugins/common/inject/platform.js';
// import User from '@/utils/user';
import ResUtil from '~/plugins/common/inject/resUtil.js';
import DateUtil from '~/plugins/common/inject/dateUtil.js';
import '@/api/api..query-open-play-sign.js';
import '@/api/api..query-quest-play-token.js';

// import '@/api/learningCenter/api.CMT1102.cmt-label-list.js';
/*
...
export default {
    ...
    mounted() {
        ...
        wrap = document.getElementById('warp' + this.componentId);
        // 支持系统储全屏则监听全屏改变时间做样式处理
        if (api) {
            document.addEventListener(api.events.change, function () {
                if (document[api.element]) {
                    wrap.classList && wrap.classList.add('full');
                } else {
                    wrap.classList && wrap.classList.remove('full');
                }
            });
        }

        // 点击全屏按钮的回调
        window.onFullscreenProxy = (vid, fullScreen) => {
    ...
    methods: {
        initCreated() {
            let vm = this;
            vm.componentId = 'vb-video-' + DateUtil.getUUID();
            ...
            let PlayReturn = {
                changeVid(param) {
                    if (!param.videoId) {
                        this.$baseUI.alert('缺少参数vid');
                        return;
                    }
                    if (!param.type) {
                        this.$baseUI.alert('缺少参数type');
                        return;
                    }
                    let playsafeParam = {};
                    let videoId = param.videoId.trim();
                    playsafeParam.videoId = videoId;
                    playsafeParam.type = param.type;
                    // playsafeParam.bizId = param.bizId ? param.bizId : "";
                    let option = {
                        vid: param.videoId,
                        autoplay: true,
                        playsafe(vid, next) {},
                    };
                    option = vm.getOption(option, playsafeParam);
                ...
                start(cb) {
                    window.s2j_onPlayStart = async () => {
                        ...
                        if (vm.$User.isLogined()) {
                            let user = await vm.$User.getUser();
                            let UserPlayVidTimeKey = (user || {}).userId + 'UserPlayVidTimeKey';
                            vm.historyUserPlayVidTime = Number(this.$storage.get(UserPlayVidTimeKey) || 0);
                            vm.getUserPlayVidTimeTimer = setInterval(function () {
                                if (!vm.instance) return;
                                this.$storage.set(UserPlayVidTimeKey, vm.historyUserPlayVidTime + vm.instance.j2s_realPlayVideoTime());
                            }, 1000);
                        }
                    };
                    return PlayReturn;
                ...
                destroy() {
                    return vm.instance.destroy();
                },
            };
            this.PlayReturn = PlayReturn;
        },
        ready(cb) {
            ResUtil.loadJs('//player.polyv.net/resp/vod-player-drm/canary/player.js', cb);
        },
        playVid(vidObj = {}, option = {}) {
            let vm = this;
            if (!vidObj.videoId) {
                this.$baseUI.alert('缺少参数vid');
                return;
            ...
            let playsafeParam = {};
            let videoId = vidObj.videoId.trim();
            playsafeParam.videoId = videoId;
            playsafeParam.groupId = vidObj.groupId;
            playsafeParam.userId = vidObj.userId;
            playsafeParam.type = vidObj.type;
            // playsafeParam.bizId = vidObj.bizId ? vidObj.bizId : "";
            $('#' + vm.componentId).html('');
            vm.ready(() => {
                let option_ = {
                    ...
                    fullscreenProxy: false, //添加该参数后点击全屏按钮播放器将不进入全屏，需要自行处理全屏
                    flashvars: {
                ...
                vm.getOption(option_, playsafeParam).then((opt) => {
                    console.log(opt, '@@@#');
                    vm.instance = polyvPlayer(opt);
                });
            });
            return vm.PlayReturn;
        },

        getOption(option, playsafeParam) {
            /**
             ...
             */
            let data = {};
            data.vid = playsafeParam.videoId;
            data.groupId = playsafeParam.groupId;
            // data.userId = playsafeParam.userId;
            return this.getPlaySign(data.vid).then((result) => {
                Object.assign(option, {
                    playsafe: result.token,
                    history_video_duration: 0.5,
                });
                return this.$User.getUser().then((user) => {
                    return Object.assign(option, {
                        code: user && user.userYoulunum ? user.userYoulunum : '',
                        pbg: false,
                    });
                });
            });
        ...
        getPlaySign(vid) {
            return this.$callApi('cc/query-open-play-sign', {
                ...
                const tokenParams = {
                    vid,
                    userYouLuNum: userInfo && userInfo.userYoulunum ? userInfo.userYoulunum : '',
