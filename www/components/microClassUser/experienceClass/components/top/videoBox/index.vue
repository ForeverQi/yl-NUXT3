    ...
    <div class="class-top-box">
        ...
        <div class="content">
            ...
            <div class="content-body">
                <div class="body-left">
                    <vb-video ref="vbVideo" v-if="showVideo" @ready="courseReady" @isFullScreen="doFullScreen" :is-overflow="true">
                        <!--如果可以试听直接播放-->
                        <!--不可以试听的必须登录并且领取-->
                ...
                <div class="body-right">
                    ...
                    ></menu-box>
                    <!-- 免费单节 相关课程-->
                    <related-box v-if="isShowRelated"></related-box>
                </div>
            </div>
...
import videoFeatureRight from '../videoFeatureRight/index.vue';
import menuBox from '../menu/menuBox/index.vue';
import relatedBox from '../relatedClass/index.vue';
...
export default {
    ...
    watch: {
        ...
        drawStatus() {
            console.warn('videoBox-drawStatus', this.drawStatus);
            this.$mcuLogin
                .checkLogin('login')
                .then(({ bindWechat }) => {
                    this.hasAttention = bindWechat;
    ...
    methods: {
        ...
        sizeOberver() {
            const dm = document.querySelector('.pv-base-control');
            if (!dm) {
                return;
            }
            dm.addEventListener(
                'transitionend',
                () => {
                    // this.loginTipStyle.bottom = `${dm.offsetHeight}px`;
                },
                false
            );
        },
            ...
            this.$refs.vbVideo
                ...
                .then((instance) => {
                    ...
                    (this.videoInstance = instance).start(() => {
                        ...
                        console.log('startstartstartstartstartstartstartstartstartstartstartstartstart');
                    });
                    this.videoInstance.over((e) => {
                        !this.isChangeing && (this.isEndAudition = true);
                        this.isChangeing = false;
                        console.log('overoveroveroveroveroveroveroveroveroveroveroveroveroverover', e, this.isChangeing);
                    });
                    this.videoScrollToIntoview();
                    setTimeout(this.sizeOberver, 5e2);
                })
                .catch((err) => {
                    console.warn(err);
        ...
        videoScrollToIntoview() {
            const vdom = document.querySelector(`#${this.videoInstance.getPlayerId()}`);
        ...
        handleLogin() {
            this.$mcuLogin.doLogin().catch((err) => {
                console.error(err);
            });
        },
        attentionService() {
            console.warn('attentionService');
            this.$mcuLogin
        ...
        handleReceive() {
            if (this.isFree) {
                this.$refs.videoFeatureRight.handleGetFree();
            } else {
                this.$refs.videoFeatureRight.handleBuy();
            }
        },
        handleLivePlay() {
        ...
        changeVideo() {
            console.warn('changeVideochangeVideochangeVideochangeVideochangeVideochangeVideochangeVideochangeVideo');
            this.isEndAudition = false;
