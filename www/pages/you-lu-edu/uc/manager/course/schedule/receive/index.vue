<template>
    <div class="tabDetails">
        <orderNull v-show="receiveList.length == 0 && loading == false" words="您还没有领取任何课单哦！"></orderNull>
        <ul class="lessonlist">
                ...
                <ul class="lessonlistul">
                    <template v-for="(course, index3) in item.courseList">
                        <li :key="index3" v-if="index3 < 3">
                            ...
                            <span :class="['lessontitle', index3 == 1 ? 'first' : '']">{{ course.courseName }}</span>
                        </li>
                    </template>
                    <li>
