 ...
 */
export function specialAppendQuestionTitleNumber(question, status = 'refresh') {
    if (status === 'refresh') {
        const root = createTempState;
        const state = root.state ? root.state : (root.state = {});
        state[1] = undefined;
    ...
    if (question.isCompositeQuestion) {
        // 小题排序
        question.number = question.questionSeq;
        // 大题排序
        question.stemNumber = question.questionStemSeq;
    }
    return question;
}
 ...
 * @param {object} aliasOptions aliasOptions
 * @returns {object} object
 */
