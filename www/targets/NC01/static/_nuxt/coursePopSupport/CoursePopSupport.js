...
!(function (e, t) {
                    ...
                    if (!a) {
                        ...
                        (a = A(
                            ...
                            (r = function (e, t, n, r, i) {
                                ...
                                var h = (k += null == p ? 1 : Math.random() || 0.1);
                                var g = d.length;
    ...
    var H = { children: !0, contents: !0, next: !0, prev: !0 };
    function O(e, t) {
    ...
    function se(e, t, n, r) {
        ...
        var c = e.nodeType && (S.cssNumber[t] || ('px' !== l && +u)) && te.exec(S.css(e, t));
        if (c && c[3] !== l) {
            (u /= 2), (l = l || c[3]), (c = +u || 1);
    ...
    var Ge = { letterSpacing: '0', fontWeight: '400' };
    function Ye(e, t, n) {
        var r = te.exec(t);
    ...
    function Je(e, t, n) {
        var r = Re(e);
        var i = (!y.boxSizingReliable() || n) && 'border-box' === S.css(e, 'boxSizing', !1, r);
        var o = i;
        var a = We(e, t, r);
        ...
        S.each(['height', 'width'], function (e, u) {
            S.cssHooks[u] = {
                ...
                set: function (e, t, n) {
                    var r;
                    var i = Re(e);
                    var o = !y.scrollboxSize() && 'absolute' === i.position;
                    var a = (o || n) && 'border-box' === S.css(e, 'boxSizing', !1, i);
    ...
    S.fn.extend({
        ...
        S.fn.extend({
            ...
            toggleClass: function (i, t) {
                var o = typeof i;
                var a = 'string' === o || Array.isArray(i);
...
!(function () {
    function sendMessage(win, func, args) {
        win.postMessage(
            {
                func: func,
                args: args,
    ...
    if (typeof window.addEventListener != 'undefined') {
        ...
        window.addEventListener(
            ...
            function (e) {
                ...
                if (e.data.func == 'loginedo') {
                    console.log('question已加载~~~~~~~~~');
                    //question  iframe加载完成
                    window.CoursePopSupport.isloginedo = true;
                    return;
                }
                if (e.data.func == 'question_loaded') {
                    window.CoursePopSupport.question_loaded = true;
                    return;
                }
                if (e.data.func == 'practiceRecord_loaded') {
                    window.CoursePopSupport.practiceRecord_loaded = true;
                    return;
                }
                if (e.data.func == 'goonPaly') {
                    window.CoursePopSupport.goonPaly_paperId = e.data.args ? e.data.args.paperId : '';
                    window.PlayTimerUtil.answersheetStatus(); //改变已答题的状态
                    window.CoursePopSupport.goonPaly();
                    return;
                }
                if (e.data.func == 'backToClassroomExercise') {
                    //预览模式下的返回
                    window.CoursePopSupport.goonPaly();
                    PageUtil.navigateBack(backToClassroomExercise);
                    return;
                }
                if (window.CoursePopSupport[e.data.func] != undefined) {
                    window.CoursePopSupport[e.data.func].apply(this, e.data.args ? [e.data.args] : []);
                }
            },
            false
        );
    ...
    var token = null;
    var UserInfo = null;
    var hasLoginedCallBack = function () {
        if ($('#' + frameId1).length > 0)
            sendMessage($('#' + frameId1)[0].contentWindow, 'loginedo', {
                token: token,
                UserInfo: UserInfo,
            });
    };
    var coursewareId; //课程ID
    var coursewareSourceid; //老课程ID
    var keyframePubstatus; //状态
    ...
    var keyframeInfo = function (coursewareIds, coursewareSourceids, keyframePubstatuss) {
        //课件打点答题状态详情
        console.log('测试管理端');
        httpClient({
            api: 'cc/keyframe/relate/answersheet/info',
            params: {
                coursewareId: coursewareIds,
                coursewareSourceid: coursewareSourceids,
                keyframePubstatus: keyframePubstatuss,
            },
            onsuccess: function (data) {
                //判断课堂记录是否显示
                if (data.data.length == 0) {
                    $('.comQues').css('display', 'none'); //隐藏
                } else {
                    var paperIds = [];
                    for (var i = 0; i < data.data.length; i++) {
                        paperIds.push(data.data[i].paperId);
                    }
                    if (paperIds.length == 0) {
                        $('.comQues').css('display', 'none'); //隐藏
                    } else {
                        $('.comQues').css('display', 'flex'); //显示
                    }
                }
                //判断打点的题是否做过
                PlayTimerUtil.timeListen(data.data, keyframePubstatuss);
            },
    ...
    var frameId1 = 'CoursePopSupport_iframe1';
    var frameId2 = 'CoursePopSupport_iframe2';
    var zhezhaoId = 'CoursePopSupport_zhezhao';
    var playerId = '';
    var playerId2 = '';
    var everyPlayId = ''; //为点击每一条题的播放视频会传id
    var backToClassroomExercise = '';
    var init = function (ops) {
        coursewareId = ops.coursewareId;
        coursewareSourceid = ops.coursewareSourceid;
        keyframePubstatus = ops.keyframePubstatus;

        backToClassroomExercise = ops.backToClassroomExercise;
        if (ops.id != undefined) {
            everyPlayId = ops.id;
        } else {
            everyPlayId = '';
        }
        var player = ops.player;
        if (ops.playerId && ops.playerId[0] == '#') {
            playerId = ops.playerId.substring(1);
        } else {
            playerId = ops.playerId;
        }
        playerId2 = ops.maxPlayerId;
        // 创建两个iframe
        if ($('#' + frameId1).length == 0) {
            $('<iframe id="' + frameId1 + '">').appendTo($('#' + playerId).parent());
            $('#' + frameId1).attr('src', CoursePopSupport.frameUrl.keyExamUrl);
        }
        if ($('#' + frameId2).length == 0) {
            $(
                '<iframe id="' +
                    frameId2 +
                    '"></iframe><div id="CoursePopSupport_zhezhao" style="width: 100%; height: 100%; background: rgba(0, 0, 0, .8); position: fixed; top: 0; left: 0; display: none; z-index: 99;"></div>'
            ).appendTo($('body'));
            $('#' + frameId2).attr('src', CoursePopSupport.frameUrl.classReportUrl);
        }
        $('#' + frameId1).hide();
        $('#' + frameId2).hide();
        $('#' + zhezhaoId).hide();
        ...
        // 本功能再管理端也有用到因此，要测试管理端
        if (ops.isJumpQuestion !== false) {
            keyframeInfo(ops.coursewareId, ops.coursewareSourceid, ops.keyframePubstatus);
        }
        //监听视频的大小
        var linstenElemet = '';
        if (ops.maxPlayerId) {
            linstenElemet = document.getElementById(ops.maxPlayerId);
        } else {
            linstenElemet = document.getElementById(playerId);
        }
        var btn = new ResizeSensor(linstenElemet, function () {
            window.PlayTimerUtil.setPosition();
        });
        console.log(btn);
    };
    var ctimer = null;
    window.CoursePopSupport = {
        ops: {},
        init: function (ops) {
            init(ops);
        },
        classReportClick: function () {
            PlayTimerUtil.showMask('2');
        },
        loginedo: function () {
            //供弹窗页面获取  登录完成回调
            if (hasLogin) hasLoginedCallBack();
        },
        getCoursewareId: function () {
            //供弹窗页面调用 获取课程ID
            return coursewareId;
        },
        getToken: function () {
        ...
        goonPaly: function () {
            //供弹窗页面调用 继续播放视频
            //TODO 继续播放 并且关闭做题弹窗
            PlayTimerUtil.hideMask('1');
            // 结束打点练习
            this.ops.changeStatus && this.ops.changeStatus(false);
        },
        closeReport: function () {
            //供弹窗页面调用 关闭随堂练习
            //TODO 关闭随堂练习弹窗
            PlayTimerUtil.hideMask('2');
        },
        isloginedo: true,
        changePaperId: function (paperId) {
            //playTimerUtil调用 打点弹题的时候 要重新加载试题信息
            var that = this;
            var func = function () {
                if (!window.CoursePopSupport.isloginedo) {
                    ctimer = setTimeout(function () {
                        func();
                    }, 500);
                } else {
                    clearTimeout(ctimer);
                    sendMessage($('#' + frameId1)[0].contentWindow, 'changePaperId', paperId);
                    // 显示打点练习
                    that.ops.changeStatus && that.ops.changeStatus(true);
                }
            };
            func();
        },
        changeSection: function (status) {
            // 设置当前是否全屏
            PlayTimerUtil.setFullScreen(status);
        },
        loadClassReport: function () {
            //playTimerUtil调用 点击随堂练习 要重新加载随堂信息
            sendMessage($('#' + frameId2)[0].contentWindow, 'loadClassReport', coursewareId);
        },
    };
})();
